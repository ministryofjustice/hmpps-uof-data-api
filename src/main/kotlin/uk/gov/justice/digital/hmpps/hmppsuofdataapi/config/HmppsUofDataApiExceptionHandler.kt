...
class HmppsUofDataApiExceptionHandler {
  ...
  fun handleValidationException(e: ReportNotFoundException): ResponseEntity<ErrorResponse> = ResponseEntity
    .status(NOT_FOUND)
    .body(
      ErrorResponse(
        status = NOT_FOUND,
  ...
  fun handleAccessDeniedException(e: Exception): ResponseEntity<ErrorResponse> = ResponseEntity
    .status(FORBIDDEN)
    .body(
      ErrorResponse(
        status = FORBIDDEN,
    ...
    ).also { log.error("Access Denied exception", e) }
